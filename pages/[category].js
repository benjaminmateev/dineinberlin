...
const ListItem = ({ restaurant, content }) => {
  ...
  const mobile = restaurant.mobile || undefined
    ...
    <li key={id} className="w-full md:w-1/2 p-3">
      <div className="relative h-full flex flex-col items-start border border-sand overflow-hidden p-4 sm:p-8 lg:px-12">
        <div className="flex-auto">
          ...
          {neighbourhoods && !!neighbourhoods.length && ( <p className="font-semibold text-2xs sm:text-sm mb-2">Distritos</p>)}
          {neighbourhoods && !!neighbourhoods.length && (
            <ul className="-m-1 mb-6">
              {neighbourhoods.sort().map(neighbourhood => (
                <li
                  key={neighbourhood}
                  className="inline-block font-medium text-xs sm:text-sm bg-sand px-2 py-1 m-1"
                >
    ...
    return (
      <ul className="flex flex-wrap -m-3">
        {restaurants
          ...
          .filter(restaurant => {
            return this.props.filterNeighbourhoods && this.props.filterNeighbourhoods.length && restaurant.neighbourhoods && restaurant.neighbourhoods.length
            ...
            <div className="max-w-6xl mx-auto">
              ...
              <div className="flex flex-wrap sm:flex-no-wrap items-end -m-1 mb-6">
                <div className="w-full flex flex-wrap items-center mb-4 sm:mb-0">
                  <p className="w-full sm:w-auto font-bold m-1 mr-2">
                    {content.offersLabel}
                  </p>
                  <div className="w-full flex flex-wrap items-center mb-4 sm:mb-0 md:max-w-6xl max-w-xl">
                    {offers.sort().map(offer => {
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-auto px-3 pt-8 sm:pt-16 pb-16">
          ...
          <div className="w-full h-full flex items-center justify-center text-3xl text-pink">
            <p>Estamos trabajando para agregar entregas en tu ciudad. <a href={"/submit/" + city}>Ayúdanos agregando</a></p>
          </div>
        </main>
        <Footer />
      </div>
...
export async function getStaticPaths() {
  return {
    paths: [
      ...
      { params: {city: 'trujillo'} },
      { params: {city: 'chiclayo'} },
      { params: {city: 'cusco'} },
      { params: {city: 'piura'} },
    ], fallback: false
  }
...
export async function getStaticProps( paths ) {
  ...
  const base = await airtable(paths.params.city)
  const records = await base
    .select({
      maxRecords: 999999, // don't want to paginate...
      view: 'Grid view', // NOTE: changing the view name will break things
      fields: ['name', 'address', 'description', 'categories', 'phone','mobile', 'url', 'neighbourhoods', 'email'],
  ...
  const neighbourhoods = Array.from(
    new Set(
      restaurants.reduce( (hoods, restaurant) => {
        if(restaurant.neighbourhoods != undefined) hoods.push(restaurant.neighbourhoods)
        return hoods
      }, []).flat(Infinity)
    )
  )
  ...
  const offers = Array.from(
    new Set(
      restaurants.reduce( (o, restaurant) => {
        if(restaurant.offerings != undefined) o.push(restaurant.offerings)
        return o
      }, []).flat(Infinity)
    )
  )
